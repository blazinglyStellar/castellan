...
export default function UnifiedLedgerPage() {
  ...
  const providerUsageEvents = providerUsageQuery.data?.data ?? [];
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {isConsumer && (
        ...
        >
          {usageEvents.length > 0 ? (
            <>{formatAmount(String(totalSpend))} <CurrencyIcon /></>
          ) : "\u2014"}
        </StatCard>
      )}
      {isConsumer && (
      ...
      {isProvider && earnings && (
        ...
        >
          {formatAmount(earnings.total_earnings)} <CurrencyIcon />
        </StatCard>
      )}
      {isProvider && earnings && (
        <StatCard
        ...
        >
          {formatAmount(earnings.unsettled_earnings)} <CurrencyIcon />
        </StatCard>
      )}
    </div>
  );
    ...
    <Card>
      ...
      <CardContent>
        <p className="text-2xl font-bold tracking-tight">
          {formatAmount(balance)} <CurrencyIcon />
        </p>
        <div className="mt-3 space-y-1.5">
          <div className="flex items-center justify-between text-sm">
            <span className="text-muted-foreground">Available</span>
            <span className="font-medium">
              {formatAmount(available)} <CurrencyIcon />
            </span>
          </div>
          {reserved > 0 && (
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium">
                {formatAmount(reserved.toFixed(7))} <CurrencyIcon />
              </span>
            </div>
          )}
...
function CurrencyIcon() {
  return (
    <XlmLogo className="inline-block h-3 w-auto text-muted-foreground" />
  );
}
        ...
        <Card>
          ...
          <CardContent className="p-0">
            <Table>
              <TableHeader>
                <TableRow>
                  <SortableHead label="Type" sortKey="entry_type" />
                  <SortableHead label="Amount" sortKey="amount" />
                  <SortableHead label="Balance After" sortKey="balance_after" />
                  <TableHead className="text-xs font-medium text-muted-foreground">
                    Description
                  </TableHead>
                  <SortableHead label="Status" sortKey="status" />
                  <SortableHead label="Date" sortKey="created_at" />
                </TableRow>
              </TableHeader>
              <TableBody>
